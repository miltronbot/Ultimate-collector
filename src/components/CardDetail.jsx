import { X, Edit3, Tag, Calendar, Hash, Trophy, DollarSign, TrendingUp, TrendingDown, FileText } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/50 animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-3">
              <div>
                ...
                {card.variant && (
                  <p className="text-sm font-semibold text-primary-500 dark:text-primary-400 mt-0.5">{card.variant}</p>
                )}
              </div>

              <div className="flex items-center gap-2 flex-wrap">
                <span className={`px-3 py-1 rounded-lg text-xs font-bold text-white ${gradeClass}`}>
                  {gradeDisplay}
                </span>
                <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-400">
                  {categoryLabel}
                </span>
                {card.serialNumber && (
                  <span className="px-2.5 py-1 rounded-lg text-xs font-mono font-semibold bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-400">
                    #{card.serialNumber}
                  </span>
                )}
              </div>
              ...
              {card.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {card.tags.map(tag => (
                    <span key={tag} className="text-xs font-semibold px-2 py-0.5 rounded-md bg-surface-100 dark:bg-surface-800 text-surface-500">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          {/* Grading subgrades if available */}
          {card.gradeSubgrades && (
            <div>
          ...
          {card.notes && (
            <div>
              ...
              <p className="text-sm text-surface-600 dark:text-surface-400 leading-relaxed bg-surface-50 dark:bg-surface-800/50 rounded-xl p-4">
                {card.notes}
              </p>
            </div>
          )}
